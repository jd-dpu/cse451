...
{
  return((float)code*0.000878906);
}

float voltageToGs(float v)
{
  return (v-1.5)/0.420;
}

accel_angles calcAngles(int codex, int codey, int codez)
{
  accel_angles angles; 
  float vx,vy,vz,gx,gy,gz;

  vx = codeToVoltage(codex);
  vy = codeToVoltage(codey);
  vz = codeToVoltage(codez);
  gx = voltageToGs(vx);
  gy = voltageToGs(vy);
  gz = voltageToGs(vz);

  angles.x = atan(gx/sqrt(gy*gy + gz*gz))*57.32;
  angles.y = atan(gy/sqrt(gx*gz + gz*gz))*57.32;
  angles.z = atan(sqrt(gx*gx+gy*gy)/gz)*57.32;
...
int main (void) {
  ...
  while (1) {
    ...
    accel_angles a = calcAngles(x_val, y_val, z_val); 
